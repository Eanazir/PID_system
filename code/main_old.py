...
def is_circle(contour, min_radius, max_radius):
    ...
    if min_radius < radius < max_radius:
        ...
        if 0.7 < circularity < 1.3:  # Adjust the circularity range as needed
            return (int(x), int(y), int(radius))
    return None

initial_angle = [135, 135, 135, 135]  # Initial angles for the motors
...
min_motor = np.array([125,125,125,125])
max_motor = np.array([145,145,145,145])
...
def move_motors(pid):
    for i in range(0,4,1):
        ...
        angle = initial_angle[i] + (pid[i]/25)
...
def control_servos():
    ...
    if np.all(errors > 10):
        ...
        integral = 0
    Kd_normalized = np.copy(Kd) / (comp_time/np.mean(computation_time))
    Ka_normalized = np.copy(Ka) * (comp_time/np.mean(computation_time))
    ...
    # pid = Kp_normalized * errors + Ki * integral  + Kd_normalized * derivative/comp_time + Ka_normalized * (((derivative - lastderivative) + (lastderivative - lastlastderivative))/2.0) 
