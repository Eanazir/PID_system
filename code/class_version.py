...
selected_color = 'orange' # input().lower()
...
while True:
    ...
    if not ret:
        print("Failed to capture the image")
        break
    print(str(y_start)+ " " + str(y_end)+ " " + str(x_start)+ " " + str(x_end) )
    ...
    if ball_position_x is not None and ball_position_y is not None:
        # if x_start <= ball_position_x <= x_end and y_start <= ball_position_y <= y_end:
